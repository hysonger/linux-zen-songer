...
prepare() {
  ...
  _make ${_compiler_flags} olddefconfig

  diff ../config .config
  echo ">>> config 和留存版本相比，出现以上修改"
  read

  _make ${_compiler_flags} LSMOD=$HOME/.config/modprobed.db localmodconfig # 可选 探测记录的modules 配合modprobed-db使用

  diff ../config .config
  echo ">>> config 和留存版本相比，出现以上修改"
  read

  _make ${_compiler_flags} menuconfig # 编译前再检查一次设置
